...
impl GraylogInterface {
    pub fn new(config: Config) -> Self {
        ...
        let interface = GraylogInterface { address, port };
...
impl GraylogInterface {
    fn get_socket(&self) -> TcpStream {
        ...
        TcpStream::connect_timeout(&ip_addr, Duration::from_secs(10)).unwrap_or_else(|e| {
            panic!(
                "Could not connect to Graylog interface on: {}:{} with: {}",
            ...
            for log in logs.iter_mut() {
                ...
                match serde_json::to_string(log) {
                    Ok(json) => {
                        ...
                        socket.write_all(&json.as_bytes()).unwrap_or_else(|e| {
                            warn!("Could not send log to Graylog interface: {}", e)
                        });
                        socket.flush().unwrap_or_else(|e| {
                            warn!("Could not send log to Graylog interface: {}", e)
                        });
                    }
...
pub fn add_timestamp_field(log: &mut ArbitraryJson) -> Result<(), std::io::Error> {
    let time_value = if let Some(i) = log.get("CreationTime") {
        ...
        return Err(std::io::Error::new(
            ErrorKind::NotFound,
            "Expected CreationTime field".to_string(),
        ));
    };
    ...
    let time_string = if let Some(i) = time_value.as_str() {
        ...
        return Err(std::io::Error::new(
            ErrorKind::NotFound,
            "Could not convert timestamp field to string".to_string(),
        ));
    };

    let time = if let Ok(i) = NaiveDateTime::parse_from_str(time_string, "%Y-%m-%dT%H:%M:%S") {
        i
    } else {
        return Err(std::io::Error::new(
            ErrorKind::NotFound,
            "Could parse time of log".to_string(),
        ));
    };

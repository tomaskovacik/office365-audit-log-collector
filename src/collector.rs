...
use crate::data_structures::{
    ArbitraryJson, Caches, CliArgs, ContentToRetrieve, JsonList, RunState,
};
...
impl Collector {
    pub async fn new(
        ...
        if args.interactive {
            interfaces.push(Box::new(InteractiveInterface::new(
                interactive_sender.unwrap(),
            )));
        }
        ...
        let filters = if let Some(filter_config) = &config.collect.filter {
    ...
    async fn handle_content(&mut self, msg: String, content: ContentToRetrieve) -> usize {
        ...
        if let Ok(logs) = serde_json::from_str::<JsonList>(&msg) {
            ...
            warn!(
                "Skipped log that could not be parsed: {}",
                content.content_id
            );
            0
        }
    ...
    async fn process_graph_logs(&mut self) {
        ...
        for graph_log in graph_logs {
            ...
            let graph_content = ContentToRetrieve {
                content_type: graph_log.content_type.clone(),
...
fn initialize_channels(
    ...
    let (status_tx, status_rx): (
        ...
        Receiver<data_structures::StatusMessage>,
    ) = channel(100000);

    let (blobs_tx, blobs_rx): (Sender<(String, String)>, Receiver<(String, String)>) =
        channel(100000);

    let (blob_error_tx, blob_error_rx): (Sender<(String, String)>, Receiver<(String, String)>) =
        channel(100000);
    ...
    let blob_config = data_structures::GetBlobConfig {
        ...
        blob_error_tx: blob_error_tx.clone(),
...
fn get_available_content(
    ...
    let (
        ...
        stats_rx,
        kill_tx,
    ) = initialize_channels(api, content_types, runs, config);

    spawn_blob_collector(
        blob_config,
        content_config,
        message_loop_config,
        blobs_rx,
...
fn spawn_blob_collector(
    ...
    thread::spawn(move || {
        api_connection::get_content_blobs(blob_config, blobs_rx, known_blobs);
    });
    thread::spawn(move || {
...
pub async fn message_loop(
    mut config: data_structures::MessageLoopConfig,
        ...
        if let Ok(Some(msg)) = config.status_rx.try_next() {
            match msg {
                ...
                data_structures::StatusMessage::FinishedContentBlobs => {
                    let new_content_types =
                        state.lock().await.awaiting_content_types.saturating_sub(1);

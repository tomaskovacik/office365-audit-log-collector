...
use crate::data_structures::{
    AuthResult, CliArgs, ContentToRetrieve, GetBlobConfig, GetContentConfig, JsonList,
    StatusMessage,
};
...
impl ApiConnection {
    ...
    pub async fn set_subscription(&self, content_type: String, enable: bool) -> Result<()> {
        ...
        if !response.status().is_success() {
            let text = response.text().await?;
            let msg = format!(
                "Received error response subscribing to audit feed {}: {}",
                content_type, text
            );
            error!("{}", msg);
            return Err(anyhow!("{}", msg));
    ...
    pub async fn subscribe_to_feeds(&self) -> Result<()> {
        ...
        let mut content_types = self
            ...
            .collect
            .content_types
            .get_management_content_type_strings();
...
pub async fn get_content_blobs(
    ...
    blobs_rx
        .for_each_concurrent(config.threads, |(content_type, url)| {
            ...
            async move {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        ...
                        handle_blob_response_error(status_tx, blob_error_tx, content_type, url)
                            .await;
                    }
                }
            }
...
async fn handle_blob_response(
    ...
    blobs_tx: Sender<(String, String)>,
    mut status_tx: Sender<StatusMessage>,
    content_tx: Sender<ContentToRetrieve>,
    mut blob_error_tx: Sender<(String, String)>,
    content_type: String,
    url: String,
    known_blobs: &HashMap<String, String>,
    duplicate: usize,
    ...
    match resp.text().await {
        Ok(text) => match serde_json::from_str::<Vec<HashMap<String, Value>>>(text.as_str()) {
            Ok(i) => {
            ...
            Err(e) => {
                ...
                match blob_error_tx.send((content_type, url)).await {
                    Err(e) => {
                        ...
                        status_tx
                            .send(StatusMessage::ErrorContentBlob)
                            .await
                            .unwrap_or_else(|e| {
                                panic!("Could not send status update, channel closed?: {}", e)
                            });
                    }
                    _ => (),
                }
...
async fn handle_blob_response_paging(
    resp: &reqwest::Response,
    mut blobs_tx: Sender<(String, String)>,
    mut status_tx: Sender<StatusMessage>,
    content_type: String,
) {
    ...
    match next_or_not {
        Some(i) => {
            ...
            blobs_tx
                .send((content_type.clone(), new_url))
                .await
                .unwrap_or_else(|e| panic!("Could not send found blob, channel closed?: {}", e));
        }
        None => {
            status_tx
                .send(StatusMessage::FinishedContentBlobs)
                .await
                .unwrap_or_else(|e| panic!("Could not send status update, channel closed?: {}", e));
        }
    };
}
...
async fn handle_blob_response_content_uris(
    mut status_tx: Sender<StatusMessage>,
    mut content_tx: Sender<ContentToRetrieve>,
    content_type: String,
    content_json: JsonList,
    known_blobs: &HashMap<String, String>,
    duplicate: usize,
) {
    for json_dict in content_json.into_iter() {
        if json_dict.contains_key("contentUri") == false {
            ...
            let url = json_dict
                ...
                .unwrap()
                .to_string();
            let expiration = json_dict
                .get("contentExpiration")
                .unwrap()
                .to_string()
                .strip_prefix('"')
                .unwrap()
            ...
            if duplicate <= 1 {
                content_tx
                    .send(content_to_retrieve)
                    .await
                    .unwrap_or_else(|e| {
                        panic!("Could not send found content, channel closed?: {}", e)
                    });
                status_tx
                    .send(StatusMessage::FoundNewContentBlob)
                    .await
                    .unwrap_or_else(|e| {
                        panic!("Could not send status update, channel closed?: {}", e)
                    });
                ...
                for _ in 0..duplicate {
                    content_tx
                        .send(content_to_retrieve.clone())
                        .await
                        .unwrap_or_else(|e| {
                            panic!("Could not send found content, channel closed?: {}", e)
                        });
...
pub async fn get_content(config: GetContentConfig, content_rx: Receiver<ContentToRetrieve>) {
    content_rx
        .for_each_concurrent(config.threads, |content_to_retrieve| {
            let client = config.client.clone();
            let headers = config.headers.clone();
            ...
            async move {
                match client
                    .get(content_to_retrieve.url.clone())
...
async fn handle_content_response(
    resp: reqwest::Response,
    mut result_tx: Sender<(String, ContentToRetrieve)>,
    mut status_tx: Sender<StatusMessage>,
    ...
    match resp.text().await {
        Ok(json) => {
            result_tx
                .send((json, content_to_retrieve))
                .await
                .unwrap_or_else(|e| panic!("Could not send status update, channel closed?: {}", e));
            status_tx
                .send(StatusMessage::RetrievedContentBlob)
                .await
                .unwrap();
        }
        Err(e) => {
